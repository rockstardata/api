...
// Mapeo de columnas: origen (dwh.organizations) -> destino (public.organization)
const columnMap = [
...
async function syncOrganization() {
  ...
  try {
    ...
    // 1. Leer datos de dwh.organizations
    console.log('📥 Leyendo datos de dwh.organizations...');
    const { rows } = await externalClient.query('SELECT * FROM dwh.organizations');
    console.log(`✅ ${rows.length} registros leídos de dwh.organizations`);

    // Eliminar línea de borrado: await localClient.query('DELETE FROM organization');
    // Eliminar línea de borrado: await localClient.query('DELETE FROM company');
    // Insertar datos en public.organization (usando el mapeo)
    if (rows.length === 0) {
    ...
    const insertQuery = `INSERT INTO organization (${colNames}) VALUES (${valuePlaceholders}) ON CONFLICT (id) DO NOTHING`;

    console.log('⬇️ Insertando datos en public.organization (con mapeo de columnas)...');
    for (const row of rows) {
      const values = columnMap.map(m => {
        if (m.target === 'createdAt') {
          // Si insert_date es null, usar fecha actual
          return row['insert_date'] ? row['insert_date'] : new Date();
        }
    ...
    console.log(`✅ ${rows.length} registros insertados en public.organization`);

    console.log('🎉 Sincronización de organization completada con éxito.');
  } catch (error) {
